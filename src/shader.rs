...
impl Shader {
    ...
    fn is_compiled(&self) -> bool {
        ...
        unsafe { gl::GetShaderiv(self.handle, gl::COMPILE_STATUS, &mut success); }
    ...
    fn get_shader_info_log(&self) -> Option<String> {
        ...
        unsafe {
            ...
            if success != gl::TRUE as GLint {
                gl::GetShaderInfoLog(self.handle, 512, ptr::null_mut(), info_log.as_mut_ptr() as *mut GLchar);
    ...
    pub fn compile(&self, source: String) -> anyhow::Result<()> {
        ...
        if !self.is_compiled() {
            if let Some(info_log) = self.get_shader_info_log() {
                anyhow::bail!("Shader compilation failed: {}", info_log);
            } else {
